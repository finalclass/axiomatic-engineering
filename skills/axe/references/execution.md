...
- `worker.runner`: required in delegated mode; `opencode` using [the OpenCode adapter](opencode.md), or `grok` using [the Grok adapter](grok.md). Other runners need an explicit adapter before use.
...
2. Map the entire delta to existing service boundaries, including affected clients. Previously implemented parts of the delta still require verification evidence. Use architecture and contracts to identify dependencies. Do not create services to fit tasks. Order tasks by dependency where possible; report a contract/architecture contradiction rather than inventing an order around it.
...
4. Prepare one task per affected service, with a fresh session per service. Keep prompts/logs in a unique temporary run directory outside `docs/` and freeze; record its path in the sync report. Serialize all workers and build/test processes in the same checkout.
...
- The relevant architecture edges and dependency contracts/types, including exact relevant operations and schemas from linked OpenAPI artifacts; code paths needed for this service. Shared contracts have a designated owner and are read-only to this worker unless explicitly assigned.
...
Preserve full logs and actual token/cost data when exposed by the runner; do not infer savings from prompt size. Bound individual waits to at most 60 seconds.
