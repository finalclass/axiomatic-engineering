...
let () =
  ...
  Printf.printf
    ...
    system.name
    (List.length system.axioms)
    (List.length system.label_defs) ;
  ...
  if needs_http config all_tasks
  then begin
  ...
  let run task =
    ...
    Printf.printf
      ...
      model_alias ;
    match
      run_task ~mu ~config ~code_dir ~quiet ~total_cost ?provider:!provider task
    with
    | Ok text ->
        record_outcome
          ~task
          ~status:"ok"
          ~output:(String.sub text 0 (min 200 (String.length text))) ;
        (text, true)
    | Error msg ->
        Printf.printf "FAILED: %s\n%!" msg ;
  ...
  then begin
    ...
    List.iter
      (fun (task, (text, ok)) ->
        if ok
        then
          begin if validation_failed text
          then begin
            ...
            outcomes :=
              List.map
                (fun (o : Sync_result.task_outcome) ->
                  if
                    o.axiom_id = task.Types.axiom_id
    ...
    while !still_failing <> [] && !cycle < config.max_cycles do
      ...
      List.iter
        (fun task ->
          ...
          then new_failures := task.Types.axiom_id :: !new_failures )
        fix_valid_tasks ;
