...
let example_path = Filename.concat (Sys.getcwd ()) "example"
...
let rec rm_rf path =
  ...
  then
    begin if Sys.is_directory path
    then begin
      Array.iter
...
let () =
  describe "Loader" (fun () ->
      ...
      it "follows links from main.md to 5 axiom files" (fun () ->
          ...
          | Ok system ->
              let ids =
                List.map (fun (a : Types.axiom) -> a.id) system.axioms
              in
      ...
      it "parses sections of frontend-design.md with correct anchors" (fun () ->
          ...
          | Ok system ->
              let fd =
                List.find
                  (fun (a : Types.axiom) -> a.id = "frontend-design.md")
                  system.axioms
              in
              let anchors =
                List.map (fun (s : Types.section) -> s.anchor) fd.sections
              in
      ...
      it "parses file-level labels on infrastructure.md" (fun () ->
          ...
          | Ok system ->
              ...
              let infra =
                List.find
                  (fun (a : Types.axiom) -> a.id = "infrastructure.md")
                  system.axioms
              in
      ...
      it "cascades global labels from ## Axioms to child axioms" (fun () ->
          ...
          | Ok system ->
              ...
              let a =
                List.find (fun (a : Types.axiom) -> a.id = "a.md") system.axioms
              in
      ...
      it
        "resolves glossary key in @satisfaction(satisfaction-level) to 0.7"
        (fun () ->
          match Loader.load ~axioms_dir:example_axioms_dir with
          | Error msg -> failwith msg
          | Ok system -> (
              let ui_label =
                List.find
                  (fun (l : Types.label_def) -> l.name = "ui")
                  system.label_defs
              in
              let threshold =
                List.find_map
      ...
      it "extracts markdown links from content" (fun () ->
          ...
          expect (List.mem "./foo.md" links || List.mem "foo.md" links)
          |> to_be_true ) ;
      ...
      it "parses glossary entries from main.md content" (fun () ->
          ...
          let first =
            List.find
              (fun (e : Types.glossary_entry) -> e.term = "testing")
              entries
          in
  ...
  describe "Consistency" (fun () ->
          ...
          | Error errs ->
              let has_missing =
                List.exists
                  (fun e ->
                    match e with
                    | Consistency.Missing_link {target; _} ->
      ...
      it "detects missing glossary key referenced by label" (fun () ->
          let system : Types.axiom_system =
            ...
            ; label_defs=
                [ { name= "ux"
                  ; phases= [Satisfaction (-1.0)]
                  ; (* sentinel value for unresolved glossary keys *)
                    markers= [Browser]
                  ; model_class= None
                  ; description= "" } ]
            ; global_labels= []
      ...
      it "formats error messages correctly" (fun () ->
          let msg =
            Consistency.error_to_string
              (Missing_link {from_axiom= "a.md"; target= "b.md"})
          in
          ...
          let msg3 =
            Consistency.error_to_string (Label_without_phase "broken")
          in
...
let () =
  describe "Snapshot" (fun () ->
      ...
      it "create_snapshot copies axioms to .axioms/current" (fun () ->
          ...
          let current_file =
            Filename.concat
              (Filename.concat (Filename.concat dir ".axioms") "current")
              "test.md"
          in
      ...
      it "save_freeze copies current to freeze" (fun () ->
          ...
          let freeze_file =
            Filename.concat (Filename.concat dotaxioms "freeze") "a.md"
          in
...
let () =
  describe "Planner" (fun () ->
      ...
      it
        "implementation tasks include [test] but exclude [ui] blocks"
        (fun () ->
      ...
      it "validation tasks include [test] with Code markers" (fun () ->
          ...
          let has_code_marker =
            List.exists
              (fun (t : Types.task) -> List.mem Types.Code t.label.markers)
              test_val
          in
      ...
      it
        "satisfaction tasks include [ui] with Browser marker and threshold 0.7"
        (fun () ->
          let tasks = Planner.satisfaction_tasks test_system all_changed in
          let ui_tasks =
            List.filter (fun (t : Types.task) -> t.label.name = "ui") tasks
          in
          expect (List.length ui_tasks) |> to_be_greater_than 0 ;
          let has_browser =
            List.exists
              (fun (t : Types.task) -> List.mem Types.Browser t.label.markers)
              ui_tasks
          in
          expect has_browser |> to_be_true ;
          let has_satisfaction =
            List.exists
              (fun (t : Types.task) ->
                match t.phase with
                | Types.Satisfaction _ -> true
                | _ -> false )
              ui_tasks
          in
      ...
      it "validation-only label is holdout for implementation" (fun () ->
          ...
          let has_holdout =
            List.exists (fun (t : Types.task) -> t.label.name = "holdout") tasks
          in
      ...
      it "filter_content returns content" (fun () ->
          ...
          in
          let filtered =
            Planner.filter_content ~allowed_labels:["test"] axiom
          in
      ...
      it "maps model class to alias from config" (fun () ->
          ...
          expect (Types.model_alias_of_class config Smart)
          |> to_equal_string "m2.7" ;
          expect (Types.model_alias_of_class config Balanced)
          |> to_equal_string "m2.7" ;
          expect (Types.model_alias_of_class config Fast)
          |> to_equal_string "k2.5" ) )
...
let () =
  describe "Markers" (fun () ->
      let example_index_content =
        read_file (Filename.concat example_code_dir "index.html")
      in
      ...
      it "detects unpaired closing /@axiom marker" (fun () ->
          ...
          | Error errs ->
              let has_unpaired_close =
                List.exists
                  (fun e ->
                    match e with
                    | Markers.Unpaired_close _ -> true
                    | _ -> false )
                  errs
              in
      ...
      it "formats marker errors correctly" (fun () ->
          let msg =
            Markers.error_to_string
              (Unpaired_open {file= "test.html"; marker= "foo.md"})
          in
          ...
          let msg2 =
            Markers.error_to_string
              (Unpaired_close {file= "test.html"; marker= "bar.md"})
          in
          expect msg2 |> to_contain "bar.md" ;

          let msg3 =
            Markers.error_to_string
              (Unknown_axiom {file= "test.html"; marker= "baz.md"})
          in
          expect msg3 |> to_contain "baz.md" ;

          let msg4 =
            Markers.error_to_string (Orphaned_code {file= "test.html"; line= 42})
          in
          expect msg4 |> to_contain "42" ) ;

      (* E-strip: strip_anchor edge cases *)
      it "strip_anchor removes #fragment from marker" (fun () ->
          expect (Markers.strip_anchor "foo.md#section")
          |> to_equal_string "foo.md" ;
          expect (Markers.strip_anchor "foo.md") |> to_equal_string "foo.md" ;
          expect (Markers.strip_anchor "foo.md#") |> to_equal_string "foo.md" ;
          expect (Markers.strip_anchor "dir/foo.md#sec")
          |> to_equal_string "dir/foo.md" ) ;
      ...
      it "validates marker with #anchor against axiom file ID" (fun () ->
          ...
          | Error errs ->
              failwith
                (String.concat "; " (List.map Markers.error_to_string errs)) ) ;
          rm_rf dir ) ;
  ...
  describe "Ai_access" (fun () ->
      ...
      it "run_agent returns Ok with final text from mock provider" (fun () ->
          let provider : Ai_access.provider =
            { name= "mock"
            ; send=
                (fun ~model:_ ~system:_ ~messages:_ ~tools:_ ~max_tokens:_ ->
            ...
            ; send=
                (fun ~model:_ ~system:_ ~messages:_ ~tools:_ ~max_tokens:_ ->
                  ...
                  then
                    ...
                    ( { Ai_access.content=
                          [ Tool_use
                              {id= "tu_1"; name= "test_tool"; input= `Assoc []}
                          ]
                      ; stop_reason= "tool_use" }
                    , (100, 50) )
                  else
                    (* Second call: return end_turn *)
                    ( { Ai_access.content= [Text "final answer"]
          ...
          let provider : Ai_access.provider =
            ...
            ; send=
                (fun ~model:_ ~system:_ ~messages:_ ~tools:_ ~max_tokens:_ ->
                  ...
                  ( { Ai_access.content=
                        [ Tool_use
                            {id= "tu_loop"; name= "loop_tool"; input= `Assoc []}
                        ]
                    ; stop_reason= "tool_use" }
                  , (10, 10) ) ) }
          in
      ...
      it "default config has correct model aliases" (fun () ->
          ...
          expect c.planner |> to_equal_string "deepseek-v3.2" ;
          expect c.implementer |> to_equal_string "m2.7" ;
          expect c.smart |> to_equal_string "m2.7" ;
          expect c.balanced |> to_equal_string "m2.7" ;
          expect c.fast |> to_equal_string "k2.5" ;
          expect c.vision |> to_equal_string "llama-vision" ) ;
      ...
      it "format_stream_event parses system init event" (fun () ->
          let json =
            Yojson.Safe.from_string
              {|{"type":"system","session_id":"abc-123","model":"claude-haiku-4-5-20251001"}|}
          in
      ...
      it "format_stream_event parses assistant tool_use" (fun () ->
          let json =
            Yojson.Safe.from_string
              {|{"type":"assistant","message":{"content":[{"type":"tool_use","name":"Edit","id":"tu_1","input":{}}]}}|}
          in
      ...
      it "format_stream_event parses result event" (fun () ->
          let json =
            Yojson.Safe.from_string
              {|{"type":"result","result":"final output","total_cost_usd":0.0123,"duration_ms":1500}|}
          in
          let display, result, _cost = Ai_access.format_stream_event json in
          ( match display with
          | Some s ->
      ...
      it "dispatch Http with mock provider runs agent loop" (fun () ->
          let provider : Ai_access.provider =
            ...
            ; send=
                (fun ~model:_ ~system:_ ~messages:_ ~tools:_ ~max_tokens:_ ->
                  ( { Ai_access.content= [Text "http result"]
                    ; stop_reason= "end_turn" }
                  , (10, 5) ) ) }
          in
          let result =
...
let () =
  describe "Types" (fun () ->
      ...
      it "resolve_model_class respects label override" (fun () ->
          ...
          let cls2 =
            Types.resolve_model_class ~label_class:Fast Implementation
          in
          expect (cls2 = Types.Fast) |> to_be_true ) ;

      it "model_alias_of_class maps correctly" (fun () ->
          let cfg = Types.default_config in
          expect (Types.model_alias_of_class cfg Smart)
          |> to_equal_string "m2.7" ;
          expect (Types.model_alias_of_class cfg Balanced)
          |> to_equal_string "m2.7" ;
          expect (Types.model_alias_of_class cfg Fast) |> to_equal_string "k2.5" ) ;
...
let () =
  describe "Tools" (fun () ->
      ...
      it "edit_file replaces old_string with new_string" (fun () ->
          ...
          let result =
            Tools.edit_file ~path ~old_string:"world" ~new_string:"OCaml"
          in
          ( match result with
          | Ok () ->
              let content = Tools.read_file ~path in
      ...
      it "list_files returns file list" (fun () ->
          let files =
            Tools.list_files ~glob:"*.md" ~base_dir:example_axioms_dir
          in
      ...
      it "execute dispatches tool calls by name" (fun () ->
          let result =
            Tools.execute
              ~base_dir:"/tmp"
              "read_file"
              (`Assoc [("path", `String "/tmp/nonexistent")])
          in
...
let () =
  describe "Integration" (fun () ->
      it
        "full pipeline: load → consistency → markers → planner on example/"
        (fun () ->
          (* Step 1: Load *)
          let system =
            match Loader.load ~axioms_dir:example_axioms_dir with
          ...
          | Error errs ->
              failwith
                ( "consistency: "
                ^ String.concat "; " (List.map Consistency.error_to_string errs)
                ) ) ;
          ...
          let all_changes =
            List.map
              (fun (a : Types.axiom) -> (a.id, Types.Added))
              system.axioms
          in
          ...
          let impl_labels =
            List.map (fun (t : Types.task) -> t.label.name) impl
          in
          let valid_labels =
            List.map (fun (t : Types.task) -> t.label.name) valid
          in
          let satisfy_labels =
            List.map (fun (t : Types.task) -> t.label.name) satisfy
          in
      ...
      it "glossary parses correctly from example/ main.md" (fun () ->
          ...
          | Ok system -> (
              ...
              let satisfaction =
                List.find_opt
                  (fun (e : Types.glossary_entry) ->
                    e.term = "satisfaction-level" )
                  system.glossary
              in
              ( match testing with
...
let () =
  describe "Cost tracking" (fun () ->
      ...
      it
        "format_stream_event extracts cost from result event with usage"
        (fun () ->
          let json =
            Yojson.Safe.from_string
              {|{"type":"result","result":"out","total_cost_usd":0.05,"duration_ms":100,"usage":{"input_tokens":1000,"output_tokens":500}}|}
          in
      ...
      it
        "format_stream_event returns None cost when no total_cost_usd"
        (fun () ->
          let json =
            Yojson.Safe.from_string
              {|{"type":"result","result":"out","duration_ms":100}|}
          in
          let _display, _result, cost = Ai_access.format_stream_event json in
      ...
      it "config defaults: max_cycles=3" (fun () ->
          let cfg = Types.default_config in
...
let () =
  describe "Semantic consistency" (fun () ->
      it
        "check_semantic returns Ok when dispatch returns NO_CONTRADICTIONS"
        (fun () ->
      ...
      it
        "check_semantic returns Error when dispatch finds contradictions"
        (fun () ->
          let dispatch ~system:_ ~prompt:_ =
            Ok "Contradiction: A conflicts with B"
          in
          let result = Consistency.check_semantic ~dispatch "axiom content" in
          match result with
...
let () =
  describe "Sync result" (fun () ->
      ...
      it "write creates .axioms/sync-result.md" (fun () ->
          ...
          let summary : Sync_result.sync_summary =
            {total_cost= 0.0; outcomes= []; mode= "full"}
          in
          Sync_result.write ~project_path:dir summary ;
          let path =
            Filename.concat (Filename.concat dir ".axioms") "sync-result.md"
          in
