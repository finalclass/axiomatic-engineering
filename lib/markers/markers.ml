...
let extract_open_marker (line : string) : string option =
  ...
  List.find_map
    (fun (open_prefix, open_suffix, _, _) ->
      let plen = String.length open_prefix in
      if
        String.length trimmed >= plen && String.sub trimmed 0 plen = open_prefix
      then begin
        ...
        let marker =
          ...
          then
            ...
            if
              String.length rest >= slen
              && String.sub rest (String.length rest - slen) slen = open_suffix
            then String.sub rest 0 (String.length rest - slen) |> String.trim
...
let extract_close_marker (line : string) : string option =
  let trimmed = String.trim line in
  List.find_map
    (fun (_, _, close_prefix, close_suffix) ->
      let plen = String.length close_prefix in
      if
        String.length trimmed >= plen
        && String.sub trimmed 0 plen = close_prefix
      then begin
        let rest = String.sub trimmed plen (String.length trimmed - plen) in
        let marker =
          if close_suffix <> ""
          then
            ...
            if
              String.length rest >= slen
              && String.sub rest (String.length rest - slen) slen = close_suffix
            then String.sub rest 0 (String.length rest - slen) |> String.trim
            else String.trim rest
          else String.trim rest
        in
    ...
    List.concat_map
      (fun entry ->
        ...
        then
          ...
          begin if entry = "tests" then [] else collect_files_recursive path
          end
        else [path] )
      entries
  end
...
let validate_file
    ...
    (file_path : string)
    (content : string) : marker_error list =
  let lines = String.split_on_char '\n' content in
  let open_stack = ref [] in
  (* stack of (marker_name, line_number) *)
  let errors = ref [] in
  ...
  List.iteri
    (fun i line ->
      ...
      | Some marker ->
          ...
          if not (List.mem axiom_id valid_ids)
          then errors := Unknown_axiom {file= file_path; marker} :: !errors ;
        ...
        | Some marker ->
            ...
            open_stack :=
              List.filter
                ...
                !open_stack ;
            if not !found
            then errors := Unpaired_close {file= file_path; marker} :: !errors
        | None -> () ) )
    lines ;
...
let validate ~(code_dir : string) (system : axiom_system) :
    (unit, marker_error list) result =
  if not (Sys.file_exists code_dir)
  then Ok ()
  else begin
...
let validate_exn code_dir (system : Types.system) : unit =
  let code_dir = Eio.Path.native_exn code_dir in
  if Sys.file_exists code_dir
  then
    match validate ~code_dir system with
