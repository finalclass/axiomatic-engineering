...
let clear_dir (path : string) : unit =
  ...
  then run_cmd_unit (Printf.sprintf "rm -rf %s/*" (Filename.quote path))
...
let create_snapshot ~(project_path : string) : unit =
  ...
  run_cmd_unit
    (Printf.sprintf
       ...
       (Filename.quote axioms_dir)
       (Filename.quote current_dir) )
...
let strip_snapshot_prefix (path : string) : string =
  ...
  let try_strip marker =
    ...
    if plen > mlen && String.sub path 0 mlen = marker
    then Some (String.sub path mlen (plen - mlen))
    else
      ...
      let rec search i =
        if i + smlen > plen
        then None
        else if String.sub path i smlen = slash_marker
        then Some (String.sub path (i + smlen) (plen - i - smlen))
        else search (i + 1)
      in
      search 0
  in
...
let classify_only_in_dir (dir : string) : [`Freeze | `Current | `Unknown] =
  if dir = "freeze" || ends_with dir "/freeze"
  then `Freeze
  else if dir = "current" || ends_with dir "/current"
  then `Current
  else `Unknown
...
let parse_diff_output (diff_output : string) : (string * axiom_change) list =
  ...
  then []
    ...
    List.iter
      (fun line ->
        if String.length line >= 4 && String.sub line 0 4 = "diff"
        then begin
          flush () ;
        ...
        else if String.length line >= 8 && String.sub line 0 8 = "Only in "
        then begin
          ...
          | Some cp -> (
              ...
              let name =
                String.trim
                  (String.sub rest (cp + 1) (String.length rest - cp - 1))
              in
        ...
        else if String.length line >= 4 && String.sub line 0 4 = "--- "
        then
          begin if String.length line >= 13 && String.sub line 4 9 = "/dev/null"
          then is_new_file := true
          end
        else if String.length line >= 4 && String.sub line 0 4 = "+++ "
        then
          begin if String.length line >= 13 && String.sub line 4 9 = "/dev/null"
          then is_deleted_file := true
          end
        else if String.length line >= 1
        then begin
          (* Look for changed section headings in context *)
          let c = line.[0] in
          if (c = '+' || c = '-') && String.length line > 1
          then begin
            ...
            then begin
              ...
              if not (List.mem anchor !current_sections)
              then current_sections := anchor :: !current_sections
            end
          end
        end )
      lines ;
...
let diff ~(project_path : string) : (string * axiom_change) list option =
  ...
  let current_dir = Filename.concat dot_axioms "current" in
  if not (Sys.file_exists freeze_dir)
  then None (* No freeze = first run = full sync *)
  else begin
    let cmd =
      Printf.sprintf
        "diff -ru %s %s 2>/dev/null || true"
        (Filename.quote freeze_dir)
        (Filename.quote current_dir)
    in
...
let save_freeze ~project_path : unit =
