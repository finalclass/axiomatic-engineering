...
let check_semantic
    ...
    (axiom_content : string) : (unit, string) result =
  let system =
  ...
  let prompt =
    Printf.sprintf
      "Analyze these axioms for contradictions:\n\n%s"
      axiom_content
  in
...
let error_to_string = function
  | Missing_link {from_axiom; target} ->
      Printf.sprintf
        ...
        from_axiom
        target
  | Duplicate_name name -> Printf.sprintf "Duplicate axiom name: '%s'" name
