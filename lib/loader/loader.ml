...
let parse_phase (s : string) : phase option =
  ...
  then
    ...
    | None -> Some (Satisfaction (-1.0))
    (* placeholder, resolved via glossary *)
...
let extract_satisfaction_key (s : string) : string option =
  if String.length s > 14 && String.sub s 0 14 = "@satisfaction("
  then
    let inner = String.sub s 14 (String.length s - 15) in
    match float_of_string_opt inner with
    | Some _ -> None
...
let parse_label_heading ?(glossary : glossary_entry list = []) (heading : string)
    : label_def option =
            ...
            (fun p ->
              ...
              | Satisfaction -1.0 -> (
                  ...
                  | Some k -> (
                      let value =
                        List.find_opt
                          (fun (e : glossary_entry) ->
                            String.lowercase_ascii e.term
                      ...
                      | None -> Satisfaction (-1.0)
                      (* keep placeholder — consistency will catch *) )
                  | None -> Satisfaction 0.7 )
              | other -> other )
            phases
        in
...
let parse_glossary (content : string) : glossary_entry list =
  ...
  List.filter_map
    (fun line ->
      ...
      if String.length line >= 4 && String.sub line 0 2 = "**"
      then
        ...
        | Some i when i + 1 < String.length line && line.[i + 1] = '*' ->
            ...
            let definition =
              ...
              if
                String.length rest >= 4 && String.sub rest 0 4 = "\xe2\x80\x94 "
              then
                (* em dash UTF-8 *)
                String.sub rest 4 (String.length rest - 4) |> String.trim
              else if
                String.length rest >= 3 && String.sub rest 0 3 = "\xe2\x80\x94"
              then String.sub rest 3 (String.length rest - 3) |> String.trim
              else if String.length rest >= 2 && String.sub rest 0 2 = "—"
              then String.sub rest 2 (String.length rest - 2) |> String.trim
              else if String.length rest >= 3 && String.sub rest 0 3 = " — "
              then String.sub rest 3 (String.length rest - 3) |> String.trim
              else rest
            in
            if term <> "" && definition <> ""
            then Some {term; definition}
            else None
        | _ -> None
      else None )
    lines
    ...
    then begin
      ...
      | Some j ->
          ...
          if
            ...
            && String.sub path (String.length path - 3) 3 = ".md"
...
let parse_inline_labels (line : string) : (string * float option) list =
  ...
  while !i < len do
    if line.[!i] = '['
    then
      begin match String.index_from_opt line (!i + 1) ']' with
      | Some j ->
...
let parse_axiom_file ~(id : string) (content : string) : axiom =
  ...
  let flush_section () =
    ...
    | None -> Buffer.clear current_content
  in

  List.iter
    (fun line ->
      if String.length line >= 2 && String.sub line 0 2 = "# " && !name = ""
      then begin
        name := String.sub line 2 (String.length line - 2) |> String.trim ;
        in_header_area := true
      end
      else if String.length line >= 3 && String.sub line 0 3 = "## "
      then begin
        flush_section () ;
        let heading =
...
let parse_label_defs ?(glossary : glossary_entry list = []) (content : string) :
    label_def list =
  let lines = String.split_on_char '\n' content in
  let defs = ref [] in
  ...
  let flush () =
    ...
    | Some ld ->
        ...
        current_def := None
    | None -> Buffer.clear desc_buf
  in

  List.iter
    (fun line ->
      if String.length line >= 4 && String.sub line 0 4 = "### "
      then begin
        flush () ;
        let heading =
    ...
    (fun line ->
      ...
      then begin
        ...
        let is_link_line =
          ...
          idx + 1 < String.length trimmed && trimmed.[idx + 1] = '('
        in
        if not is_link_line then labels := !labels @ List.map fst parsed
      end )
    lines ;
  !labels
...
let axiom_id_of_path (rel_path : string) : string = rel_path
...
let load ~(axioms_dir : string) : (axiom_system, string) result =
  ...
  | Some main_content ->
      ...
      let rec load_axiom (rel_path : string) =
        if not (Hashtbl.mem loaded rel_path)
        then begin
          ...
          | Some content ->
              ...
              let sections =
                List.map
                  (fun (s : section) -> {s with labels= file_labels @ s.labels})
                  axiom.sections
              in
              let axiom = {axiom with labels= file_labels; sections} in
              axioms := axiom :: !axioms ;
              (* Follow links from this axiom, resolving relative paths *)
              let sub_links = extract_links content in
              let resolved_links =
                List.map
                  (fun link ->
                    let base_dir = Filename.dirname rel_path in
                    let link' =
                      if String.length link >= 2 && String.sub link 0 2 = "./"
                      then String.sub link 2 (String.length link - 2)
                      else link
                    in
                    if base_dir = ""
                    then link'
                    else Filename.concat base_dir link' )
                  sub_links
              in
              List.iter load_axiom resolved_links
        end
      in
