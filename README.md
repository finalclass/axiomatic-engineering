...
This repository is the manifesto plus the **axe** skill. There is no custom harness. Any coding agent that can follow a skill is the compiler.

## Install

```
npx skills add finalclass/axiomatic-engineering
```

That copies [skills/axe/SKILL.md](skills/axe/SKILL.md) into Claude Code, Codex, Cursor, and other agents the CLI detects. Then tell the agent to load `axe`.
...
No custom compiler. The axe skill **is** the process. The agent in your repo follows [skills/axe/SKILL.md](skills/axe/SKILL.md).

Two entry points:

1. **Change requested in chat** — the agent proposes a unified diff of the spec. You accept the hunks. It writes the spec and **stops**. No code yet.
2. **`axe sync` / you already edited the spec** — the agent diffs against freeze, implements only that delta, verifies what the labels require, then updates freeze.

In the skill the working names are `docs/` (specification tree) and `.axe/freeze` (last successful snapshot). Same roles as `axioms/` and `.axioms/` in the layout above.
